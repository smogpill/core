...
coFORCE_INLINE coInt32x4 coFullMask(const coInt32x4& m)
{
	return coBitCast<coInt32x4>(_mm_cmpeq_epi32(coBitCast<__m128i>(m), coBitCast<__m128i>(m)));
}
...
coFORCE_INLINE coInt32x4 operator~(const coInt32x4& m)
{
	return coBitCast<coInt32x4>(_mm_xor_si128(coBitCast<__m128i>(m), coBitCast<__m128i>(coFullMask(m))));
}